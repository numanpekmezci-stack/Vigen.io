        ...
        {videoUrl && state === "done" && (
          <div className="flex flex-col items-center mb-4">
            ...
            <div className="flex gap-2 mt-3">
              <a
                href={videoUrl}
                download={`vigen_${Date.now()}.${outputType === "image" ? "png" : "mp4"}`}
                target="_blank"
                className="px-5 py-2 bg-[#c8f135] text-[#080809] text-xs font-bold rounded-lg hover:bg-[#b5dd2e] transition flex items-center gap-1.5"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                Download
              </a>
              <button
                onClick={() => { setVideoUrl(null); setState("idle"); setPrompt(""); }}
                className="px-5 py-2 border border-[#1f1f24] text-xs text-[#888] rounded-lg hover:border-[#333] transition"
              >
                New Generation
              </button>
            </div>
